...
`agent-exec-gateway` is a cross-platform command execution gateway for agent runtimes and tool orchestration systems.
...
It does **not** replace dedicated filesystem operation controls. For mutating file operations, use `safe-fs-tools` in combination with this gateway.
